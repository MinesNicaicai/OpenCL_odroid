...
int status;
 ...
 // Input buffers.
  ...
  printf ("GPU took %lld nanoseconds to write data to the buffers.\n", chrono::duration_cast<chrono::nanoseconds>(chrono_end - chrono_start).count() );
  ...
  printf ("GPU took %lld nanoseconds to write data to the buffers. (getEventProfiling)\n", (command_end - command_start));
  ...
  // printf ("command start = %llu nanoseconds.\n", command_start);
  // printf ("command end = %llu nanoseconds.\n", command_end);

  chrono_start = chrono::steady_clock::now();
    status = clEnqueueReadBuffer(queue, output_buf, CL_TRUE,
        0, N* sizeof(float), output, 1, &kernel_event, &finish_event);
  chrono_end = chrono::steady_clock::now();
  printf ("GPU took %lld nanoseconds to read data from the buffers.\n", chrono::duration_cast<chrono::nanoseconds>(chrono_end - chrono_start).count() );
  clGetEventProfilingInfo(finish_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &command_start, NULL);
  clGetEventProfilingInfo(finish_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &command_end, NULL);
  printf ("GPU took %lld nanoseconds to read data from the buffers. (getEventProfiling)\n", (command_end - command_start));
